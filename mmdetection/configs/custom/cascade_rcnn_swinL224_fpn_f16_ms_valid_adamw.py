...
test_pipeline = [
    ...
    dict(
        ...
        img_scale=[(512, 512), (1024, 1024), (1536, 1536)], #(512, 512), # Resize
        flip=True,   #False,
        transforms=[
...
data = dict(
    ...
    train=dict(
        type=dataset_type,
        ann_file=data_root + 'cv_train_1.json', ## train annotation file 위치
        img_prefix=data_root, # + 'train2017/', ## data root 위치
        classes = classes, # classes 추가
        pipeline=train_pipeline), ## classes 추가
    val=dict(
        type=dataset_type,
        ann_file=data_root + 'cv_val_1.json', # + 'val.json', ## validation annotation file 위치
    ...
    hooks=[
        ...
        dict(type='WandbLoggerHook', #interval=1000,
            init_kwargs=dict(
                ...
                name = "cascade_rcnn_swinL224_fpn_f16_ms_valid_adamw" #'실험할때마다 RUN에 찍히는 이름'
            ),
            )
...
work_dir = './work_dirs/cascade_rcnn_swinL224_fpn_f16_ms_valid_adamw'
