...
# model settings
# aug4 : CLAHE, ColorJitter, Mosaic, RandomSizedBBoxSafeCrop
model = dict(
    type='CascadeRCNN',
    backbone=dict(
...
train_pipeline = [
    ...
    dict(
        type='Albu',
        transforms=[
            dict(type='RandomSizedBBoxSafeCrop', height=1024, width=1024, p=0.5),
        ],
        bbox_params=dict(
            type='BboxParams',
            format='pascal_voc',
    ...
    dict(
        type='Albu',
        transforms=[
            dict(type='ColorJitter', contrast=7.0, p = 0.8),
            dict(type='CLAHE', p = 1)],
        bbox_params=dict(
            type='BboxParams',
            format='pascal_voc',
...
train_pipeline = [
    ...
    dict(type='Albu',
    transforms=[
        dict(type='ColorJitter', contrast=7.5, p = 0.5),
        dict(type='CLAHE', p = 1),
    ...
    hooks=[
        ...
        dict(type='WandbLoggerHook', #interval=1000,
            init_kwargs=dict(
                ...
                name = "cascade_rcnn_swinL224_fpn_f16_ms_aug_4_epoch30" #'실험할때마다 RUN에 찍히는 이름'
            ),
            )
...
work_dir = '/opt/ml/detection/baseline/mmdetection/work_dirs/cascade_rcnn_aug_4'
